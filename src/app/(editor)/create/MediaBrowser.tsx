...
const MediaBrowser: React.FC<MediaBrowserProps> = ({ onSelect, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/30 backdrop-blur-sm animate-[fade-in_0.15s_ease-out]">
      <div className="w-full max-w-5xl p-6 bg-[var(--color-editor-surface)] rounded-2xl shadow-2xl flex flex-col max-h-[90vh] border border-[var(--color-editor-border)]">
        ...
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-[var(--color-editor-border)]">
          <div>
            <h2 className="text-xl font-bold text-[var(--color-editor-text)]">Media Library</h2>
        ...
        <div className="flex-1 overflow-y-auto min-h-[400px]">
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full space-y-4">
              ...
              <p className="text-sm text-[var(--color-editor-muted)] font-medium">Loading your media...</p>
            </div>
          ) : error ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3">
              <div className="p-4 bg-[var(--color-editor-elevated)] rounded-2xl">
                <ImageIcon size={40} className="text-[var(--color-editor-muted)]" />
              </div>
              <p className="text-base font-medium text-[var(--color-editor-secondary)]">No media found</p>
              <p className="text-sm text-[var(--color-editor-muted)]">Upload some images first to see them here.</p>
            </div>
          ) : (
