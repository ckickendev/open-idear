        ...
        <div className="w-4/5 space-y-4">
          {data.map((article, idx) => (
            <div
              key={idx}
              className={
                "grid md:grid-cols-5 gap-y-4 bg-background rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow"
              }
            >
              ...
              <div className={`p-6 flex flex-col justify-center md:col-span-3`}>
                <CategoryLinkCustom
                  className="text-yellow-600 text-xs font-semibold mb-2"
                  slug={article?.category?.slug}
                  name={article?.category?.name}
                />
                <PostLinkCustom
                  className={
                    "text-2xl font-bold mb-4 hover:underline line-clamp-2"
                  }
                  slug={article?.slug}
                  name={article?.title}
                />
        ...
        <div className="w-1/5">
          ...
          {allCategory.map((cat: any, idx: number) => (
            <div key={idx} className="mb-6">
              <CategoryLinkCustom
                className="text-heading bg-gradient-to-r from-gray-200 to-green-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-base text-sm px-4 py-2.5 leading-5 mb-2 mr-2"
                slug={cat.slug}
                name={cat.name}
              />
            </div>
