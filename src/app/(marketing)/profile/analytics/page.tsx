      ...
      try {
        ...
        if (res.success) {
          ...
          const totalStudents = allCourses.reduce(
            (acc: number, c: any) =>
              acc + (c.enrolledStudents?.length || c.students || 0),
            0,
          );
      ...
      <div className="flex items-end gap-3 h-40 mt-4">
        {data.map((d, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-2">
            ...
            />
            <span className="text-[10px] text-muted-foreground dark:text-muted-foreground text-center leading-tight">
              {d.label}
            </span>
          </div>
        ))}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-background dark:bg-card rounded-2xl border border-border dark:border-border p-6">
          <h2 className="text-base font-bold text-foreground dark:text-white mb-1">
            Course Status
          </h2>
          ...
          {loading ? (
            <div className="h-40 bg-muted dark:bg-accent rounded-xl animate-pulse" />
          ) : (
            <BarChart
              data={[
                {
          ...
          {loading ? (
            ...
            <BarChart
              data={[
                {
                  label: "Oct",
                  value: Math.floor(stats.totalStudents * 0.3),
                  color: "bg-indigo-400",
                },
                {
                  label: "Nov",
                  value: Math.floor(stats.totalStudents * 0.5),
                  color: "bg-indigo-500",
                },
                {
                  label: "Dec",
                  value: Math.floor(stats.totalStudents * 0.65),
                  color: "bg-indigo-500",
                },
                {
                  label: "Jan",
                  value: Math.floor(stats.totalStudents * 0.8),
                  color: "bg-indigo-600",
                },
                {
                  label: "Feb",
                  value: Math.floor(stats.totalStudents * 0.9),
                  color: "bg-indigo-600",
                },
                {
                  label: "Mar",
        ...
        {loading ? (
          <div className="p-6 space-y-3">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="h-12 bg-muted dark:bg-accent rounded-lg animate-pulse"
              />
            ))}
          </div>
        ) : courses.length > 0 ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border dark:border-border">
                  <th className="text-left text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-wider px-6 py-3">
                    Course
                  </th>
                  <th className="text-left text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-wider px-6 py-3">
                    Status
                  </th>
                  <th className="text-right text-xs font-semibold text-muted-foreground dark:text-muted-foreground uppercase tracking-wider px-6 py-3">
                    Students
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border dark:divide-border/50">
                {courses.map((course) => (
                  <tr
                    key={course._id}
                    className="hover:bg-muted/30 dark:hover:bg-accent/30 transition-colors"
                  >
