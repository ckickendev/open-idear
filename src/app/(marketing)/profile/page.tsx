    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-foreground dark:text-white flex items-center gap-2">
          {greeting},{" "}
          {currentUser.name || String(currentUser.username) || "there"}! 👋
        </h1>
      ...
      <div className="bg-background dark:bg-card rounded-2xl border border-border dark:border-border p-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recentCourses.map((course: any) => (
              <Link
                ...
                className="relative group flex gap-4 p-4 rounded-xl border border-border dark:border-border bg-background dark:bg-card hover:bg-muted/30/50 dark:hover:bg-card/80 backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 hover:border-indigo-200 dark:hover:border-indigo-800 overflow-hidden"
              >
                {/* Ambient Glass Glow */}
                ...
                <div className="w-24 h-16 rounded-xl overflow-hidden bg-muted dark:bg-accent flex-shrink-0 shadow-inner">
                  {course.thumbnail?.url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <BookOpen
                        size={20}
                        className="text-muted-foreground/70 dark:text-muted-foreground transition-transform duration-500 group-hover:scale-110"
                      />
                    </div>
                  )}
                </div>
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <h3 className="text-sm font-semibold text-foreground dark:text-white truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {course.title}
                  </h3>
                  <div className="flex items-center gap-3 mt-1.5">
                    <div className="flex items-center gap-1.5 bg-muted dark:bg-accent px-2 py-0.5 rounded-md">
                      <Clock size={12} className="text-muted-foreground" />
                      <span className="text-[10px] font-medium text-muted-foreground dark:text-muted-foreground/70 uppercase tracking-wider">
                        In Progress
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      <div
                        ...
                        style={{
                          background: "conic-gradient(#6366f1 60%, #e5e7eb 0)",
                        }}
                      >
                        <div className="w-3 h-3 bg-background dark:bg-card rounded-full" />
                      </div>
          ...
          <div className="text-center py-10">
            <BookOpen
              size={36}
              className="text-muted-foreground/70 dark:text-muted-foreground mx-auto mb-3"
            />
            <p className="text-sm text-muted-foreground dark:text-muted-foreground">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {quickActions.map((action) => (
            <Link
              key={action.label}
              href={action.href}
              className="group relative flex items-center gap-4 p-5 bg-background dark:bg-card rounded-2xl border border-border dark:border-border hover:border-indigo-100 dark:hover:border-indigo-900/50 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:hover:shadow-indigo-900/20 hover:-translate-y-1 transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="relative z-10">
                <p className="text-sm font-bold text-foreground dark:text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-indigo-600 group-hover:to-blue-600 dark:group-hover:from-indigo-400 dark:group-hover:to-blue-400 transition-all duration-300">
                  {action.label}
                </p>
