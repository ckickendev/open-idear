...
const CourseCard = ({ course }: { course: Course }) => {
    ...
    return (
        <Link href={`/courses/${course.slug}`} className="group relative block w-[280px] flex-shrink-0 bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-indigo-100 shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-1">
            {/* Ambient Glass Glow */}
            ...
            <div className="px-5 pb-5 relative z-10 bg-white">
                <h3 className="font-bold text-[15px] leading-tight text-gray-900 mb-1.5 line-clamp-2 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-indigo-600 group-hover:to-blue-600 transition-colors">
                    {course.title}
                </h3>
                <p className="text-xs text-gray-500 mb-2 truncate font-medium">{course.instructor?.name}</p>
                <div className="flex items-center gap-1.5 mb-2.5">
                    <span className="text-sm font-extrabold text-amber-500">{rating.toFixed(1)}</span>
                    <RatingStars rating={rating} />
                    <span className="text-xs text-gray-400 font-medium">({students.toLocaleString()})</span>
                </div>
                <div className="flex items-center gap-3 text-[11px] text-gray-500 mb-3 font-medium">
                    <span className="flex items-center gap-1"><Clock size={12} className="text-indigo-400" /> {totalHours} giờ</span>
                    <span className="flex items-center gap-1"><BookOpen size={12} className="text-blue-400" /> {totalLectures} bài</span>
                </div>
                <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-100">
                    <div className="flex items-center gap-2">
                        {course.discountPrice && course.discountPrice > 0 ? (
                            <>
                                <span className="font-extrabold text-gray-900 text-lg">₫{course.discountPrice.toLocaleString()}</span>
                                <span className="text-xs text-gray-400 line-through">₫{course.price.toLocaleString()}</span>
                            </>
                        ) : course.price === 0 ? (
                            <span className="px-2.5 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-md">Miễn phí</span>
                        ) : (
                            <span className="font-extrabold text-gray-900 text-lg">₫{course.price.toLocaleString()}</span>
                        )}
                    </div>
                </div>
            </div>
...
const CourseRow = ({ courses }: { courses: Course[] }) => {
    ...
    return (
        <div className="relative group/row">
            <button
                onClick={() => scroll('left')}
                className="absolute -left-5 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white/90 backdrop-blur-md text-[var(--color-admin-primary)] rounded-full flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-white hover:scale-110 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100"
            >
                <ChevronLeft size={24} />
            </button>
            <div
            ...
            <button
                ...
                className="absolute -right-5 top-1/2 -translate-y-1/2 z-10 w-12 h-12 bg-white/90 backdrop-blur-md text-[var(--color-admin-primary)] rounded-full flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition-all hover:bg-white hover:scale-110 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100"
            >
        ...
        <div className="bg-white min-h-screen font-sans text-[#1c1d1f]">
                ...
                <div className="relative max-w-7xl mx-auto px-6 flex flex-col lg:flex-row items-center">
                    <div className="flex-1 py-16 lg:py-24 lg:pr-12">
                        <h1 className="text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-6">
                            Học kỹ năng mới.<br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-indigo-200">Xây dựng tương lai.</span>
                        </h1>
                        ...
                        <div className="flex max-w-xl bg-white rounded-full p-1.5 shadow-2xl shadow-indigo-900/50 backdrop-blur-sm border border-white/20">
                            <div className="flex-1 flex items-center pl-4">
                                <Search className="text-gray-400 flex-shrink-0" size={20} />
                                <input
                                    type="text"
                                    placeholder="Bạn muốn học gì hôm nay?"
                                    className="w-full py-3 px-3 text-gray-800 focus:outline-none text-base bg-transparent placeholder-gray-400"
                                    value={searchTerm}
            ...
            <section className="bg-gray-50 border-b border-gray-100 relative overflow-hidden">
                <div className="max-w-7xl mx-auto px-6 py-10 flex flex-col sm:flex-row items-center justify-center gap-8 relative z-10">
                    <span className="text-sm text-gray-500 font-bold whitespace-nowrap uppercase tracking-wider">Được tin tưởng bởi</span>
                    ...
                    <div className="flex-1 w-full relative">
                        ...
                        <div className="flex justify-around items-center gap-8 opacity-40 grayscale hover:grayscale-0 transition-all duration-500 overflow-x-auto scrollbar-hide py-2">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/2/2f/Google_2015_logo.svg" alt="Google" className="h-6 hover:scale-110 transition-transform" />
                            <img src="https://upload.wikimedia.org/wikipedia/commons/9/96/Microsoft_logo_%282012%29.svg" alt="Microsoft" className="h-6 hover:scale-110 transition-transform" />
                            <img src="https://upload.wikimedia.org/wikipedia/commons/a/a9/Amazon_logo.svg" alt="Amazon" className="h-6 hover:scale-110 transition-transform" />
                            <img src="https://upload.wikimedia.org/wikipedia/commons/7/7b/Meta_Platforms_Inc._logo.svg" alt="Meta" className="h-6 hover:scale-110 transition-transform" />
                        </div>
                    </div>
                </div>
            </section>
                    ...
                    {TOPICS.slice(0, 6).map(topic => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-300 shadow-sm ${activeTab === topic.label
                                ...
                                : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 hover:shadow-md'
                                }`}
                        >
                            {topic.label}
                        </button>
                ...
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {FEATURED_TOPICS.map(topic => (
                            <div key={topic.label} className="bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 hover:border-indigo-100 transition-all duration-300 cursor-pointer group relative overflow-hidden">
                                <div className={`absolute -right-4 -top-4 w-24 h-24 bg-gradient-to-br from-transparent to-current opacity-5 rounded-full ${topic.color} group-hover:scale-150 transition-transform duration-500`} />
                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-5 ${topic.color.replace('text-', 'bg-').replace('600', '50')} ${topic.color} group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                                    <topic.icon size={28} />
                                </div>
                                <h3 className="font-bold text-lg mb-1.5 text-gray-900 group-hover:text-[var(--color-admin-primary)] transition-colors relative z-10">{topic.label}</h3>
                                <p className="text-sm text-gray-500 font-medium relative z-10">{topic.count} khóa học</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="bg-[radial-gradient(ellipse_at_bottom,_var(--tw-gradient-stops))] from-gray-900 via-[#1c1d1f] to-black text-white py-20 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <h2 className="text-3xl font-extrabold mb-16 text-center">Tại sao nên học tại OpenIdear?</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                        <div className="text-center group">
                            ...
                            <h3 className="font-bold text-xl mb-3 text-white">Học theo tốc độ của bạn</h3>
                            <p className="text-gray-400 text-sm leading-relaxed font-medium">Truy cập mọi lúc mọi nơi. Học trên di động hay máy tính, phù hợp với lịch trình của bạn.</p>
                        </div>
                        <div className="text-center group">
                            ...
                            <h3 className="font-bold text-xl mb-3 text-white">Chứng chỉ chuyên nghiệp</h3>
                            <p className="text-gray-400 text-sm leading-relaxed font-medium">Nhận chứng chỉ hoàn thành khóa học được công nhận bởi cộng đồng và doanh nghiệp.</p>
                        </div>
                        <div className="text-center group">
                            ...
                            <h3 className="font-bold text-xl mb-3 text-white">Cộng đồng hỗ trợ</h3>
                            <p className="text-gray-400 text-sm leading-relaxed font-medium">Kết nối, thảo luận và nhận hỗ trợ từ giảng viên cùng hàng ngàn học viên khác.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-20 bg-white">
                <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center gap-16">
                    ...
                    <div className="flex-1">
                        <h2 className="text-3xl lg:text-4xl font-extrabold mb-6 text-gray-900">Trở thành giảng viên</h2>
            ...
            <section className="bg-gray-50 py-16 border-t border-gray-100">
                <div className="max-w-7xl mx-auto px-6">
                    <h2 className="text-2xl font-bold mb-8 text-gray-900">Chủ đề hàng đầu</h2>
                    <div className="flex flex-wrap gap-3">
                        {TOPICS.map(t => (
                            <Link
                                key={t.label}
                                href={`/courses/search?q=${encodeURIComponent(t.label)}`}
                                className="bg-white border border-gray-200 px-6 py-2.5 rounded-full text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-[var(--color-admin-primary)] hover:border-indigo-200 hover:shadow-sm transition-all duration-300"
                            >
                                {t.label}
                            </Link>
                        ))}
