...
import {
    BookOpen, Play, ArrowRight, Clock, Award,
    Search, Filter, Lock
} from 'lucide-react';
...
const EnrolledCourseCard = ({ enrollment }: { enrollment: EnrollmentItem }) => {
    ...
    const totalLessons = course.chapters?.reduce(
        (total, ch) => total + (ch.lessons?.length || 0), 0
    ) || 0;

    return (
        <Link
            href={`/courses/${course.slug}/learn`}
            className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 hover:border-indigo-100 transition-all duration-300"
        >
            {/* Thumbnail */}
            ...
            {/* Content */}
            <div className="p-5">
                <h3 className="font-bold text-[15px] text-gray-900 leading-tight line-clamp-2 mb-2 group-hover:text-[var(--color-admin-primary)] transition-colors">
                    {course.title}
                </h3>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-bold ${
                        progress >= 100 ? 'text-emerald-600' : 'text-[var(--color-admin-primary)]'
                    }`}>
                        {progress}%
                    </span>
                </div>
            ...
            <div className="px-5 pb-5">
                <div className="bg-gray-50 group-hover:bg-indigo-50 rounded-xl py-3 text-center transition-colors">
                    <span className="text-sm font-bold text-gray-700 group-hover:text-[var(--color-admin-primary)] flex items-center justify-center gap-2 transition-colors">
...
const MyLearningPage = () => {
    ...
    if (!currentUser?._id) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
                ...
                <p className="text-gray-500">Đăng nhập để xem các khóa học đã đăng ký.</p>
            </div>
        );
    }
    ...
    const filtered = enrollments.filter(e => {
        ...
        const matchSearch = e.course.title.toLowerCase().includes(searchTerm.toLowerCase());
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-gradient-to-r from-[var(--color-admin-primary)] via-indigo-600 to-blue-700 text-white">
                <div className="max-w-7xl mx-auto px-6 py-12">
                    <h1 className="text-3xl lg:text-4xl font-extrabold mb-2">Học tập của tôi</h1>
                    ...
                    {enrollments.length > 0 && (
                        <div className="flex gap-8 mt-8">
                            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-6 py-3 border border-white/10">
                                ...
                                <div className="text-xs text-indigo-200 font-medium">Tổng khóa học</div>
                            </div>
                            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-6 py-3 border border-white/10">
                                <div className="text-2xl font-extrabold">{inProgressCount}</div>
                                <div className="text-xs text-indigo-200 font-medium">Đang học</div>
                            </div>
                            <div className="bg-white/10 backdrop-blur-sm rounded-xl px-6 py-3 border border-white/10">
                                <div className="text-2xl font-extrabold">{completedCount}</div>
                                <div className="text-xs text-indigo-200 font-medium">Đã hoàn thành</div>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                        <div className="flex gap-2">
                            {(['all', 'active', 'completed'] as const).map(status => (
                                ...
                                >
                                    {status === 'all' ? 'Tất cả' : status === 'active' ? 'Đang học' : 'Hoàn thành'}
                                </button>
                            ))}
                        </div>
                        <div className="relative w-full sm:w-80">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--color-admin-primary)] focus:border-transparent text-sm"
                            />
                        </div>
                    </div>
                )}
