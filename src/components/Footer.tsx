"use client";
import { useTranslation } from "@/app/hook/useTranslation";
import Logo from "@/components/common/Logo";
import Image from "next/image";

export default function Footer() {
  const { t } = useTranslation();
  return (
    <footer className="px-3 mx-auto pt-4 lg:px-9 border-t border-border bg-card flex justify-center items-center flex-col">
      <div className="container px-4 grid gap-10 row-gap-6 mb-8 sm:grid-cols-2 lg:grid-cols-4">
        <div className="sm:col-span-2">
          <a href="#" className="inline-flex items-center">
            <Logo className="w-30 mr-2" />
            <span className="text-xl font-bold tracking-wide text-foreground uppercase">
              OpenIdear
            </span>
          </a>
          <div className="mt-6 lg:max-w-xl">
            <p className="text-sm text-muted-foreground">
              {t("component.footer.description")}
            </p>
          </div>
        </div>

        <div className="flex flex-col gap-2 text-sm">
          <p className="text-base font-bold tracking-wide text-foreground">
            {t("component.footer.hotcourse")}
          </p>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            UPSC - Union Public Service Commission
          </a>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            General Knowledge
          </a>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            MBA
          </a>
          <p className="text-base font-bold tracking-wide text-foreground">
            {t("component.footer.hottoppic")}
          </p>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            Human Resource Management
          </a>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            Operations Management
          </a>
          <a
            href="#"
            className="text-muted-foreground hover:text-foreground transition-colors"
          >
            Marketing Management
          </a>
        </div>

        <div>
          <p className="text-base font-bold tracking-wide text-foreground">
            {t("component.footer.ava")}
          </p>
          <div className="flex items-center gap-1 px-2">
            <a href="#" className="w-full">
              <Image
                src="https://upload.wikimedia.org/wikipedia/commons/6/67/App_Store_%28iOS%29.svg"
                alt="Appstore Button OpenIdear"
                width={40}
                height={40}
                className="h-8 w-auto"
              />
            </a>
            <a
              className="w-full"
              href="https://www.youtube.com/channel/UCo8tEi6SrGFP8XG9O0ljFgA"
            >
              <Image
                src="https://upload.wikimedia.org/wikipedia/commons/0/09/YouTube_full-color_icon_%282017%29.svg"
                alt="Youtube Button OpenIdear"
                width={40}
                height={40}
                className="h-8 w-auto"
              />
            </a>
            <a className="w-full" href="https://www.facebook.com/openidear">
              <Image
                src="https://upload.wikimedia.org/wikipedia/commons/f/fb/Facebook_icon_2013.svg"
                alt="Facebook Button OpenIdear"
                width={40}
                height={40}
                className="h-8 w-auto"
              />
            </a>
          </div>
          <p className="text-base font-bold tracking-wide text-foreground">
            {t("component.footer.contact")}
          </p>
          <div className="flex">
            <p className="mr-1 text-muted-foreground">Email:</p>
            <a
              href="#"
              title="send email"
              className="text-primary hover:underline"
            >
              opentrashtech@gmail.com
            </a>
          </div>
        </div>
      </div>

      <div className="w-full flex flex-col-reverse justify-center items-center pt-5 pb-10 border-t border-border lg:flex-row">
        <p className="text-sm text-muted-foreground">
          © Copyright 2023 Company. All rights reserved.
        </p>
      </div>
    </footer>
  );
}
