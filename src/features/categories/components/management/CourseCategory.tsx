...
const CourseCategory = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<CourseCategoryType | null>(
    null,
  );
    ...
    if (formData.name.trim()) {
      try {
        ...
        if (newCategory.success || newCategory.data?.category) {
          ...
          setCategories([
            ...categories,
            newCategory.data?.category || (newCategory.data as any),
          ]);
  ...
  const handleEditCategory = () => {
    ...
    courseCategoryApi
      .updateCourseCategory(selectedItem?._id as string, {
        name: formData.name,
      ...
      .then((response) => {
        if (response.success || response.data?.category) {
          const updatedCategory =
            response.data?.category || (response.data as any);
          setCategories(
            categories.map((cat) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Quản lý các danh mục khoá học của hệ thống
          </p>
        </div>
        <button
      ...
      <AdminSearchInput
        ...
        placeholder="Tìm kiếm danh mục khoá học..."
      />
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <TextAreaCustom
                  ...
                  rows={3}
                  placeholder="Mô tả về danh mục khoá học này"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground/80 mb-1.5">
