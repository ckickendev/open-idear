...
const Series = () => {
    ...
    if (formData.title.trim()) {
      try {
        ...
        if (newSeries.success) {
          ...
          toast.success("Thêm series thành công");
  ...
  const handleEditSeries = () => {
    changeLoad();
    if (!formData.title.trim()) {
      toast.success("Tiêu đề không được để trống");
      changeLoad();
      return;
    }
    seriesApi
      ...
      .then((response) => {
        if (response.success) {
          setSeries(
            series.map((i) =>
              i._id === selectedItem?._id ? response.data.series : i,
            ),
          );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-background rounded-xl border border-border shadow-admin-sm overflow-hidden">
        {isDataLoading ? (
          ...
          <EmptyState
            icon={BookText}
            title={searchTerm ? "Không tìm thấy series" : "Chưa có series nào"}
            description={
              searchTerm
                ? "Thử thay đổi từ khóa tìm kiếm"
                : "Bắt đầu bằng cách thêm series đầu tiên"
            }
            actionLabel={!searchTerm ? "Thêm Series" : undefined}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Link
                          ...
                          className="text-sm font-medium text-foreground hover:text-admin-primary transition-colors line-clamp-1"
                        >
                          {item.title}
                        </Link>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <Link
                        href={`/series/${item.slug}`}
                        className="text-sm font-medium text-foreground hover:text-admin-primary line-clamp-2"
                      >
                        {item.title}
                      </Link>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => confirmDelete && handleDeleteSeries(confirmDelete)}
        title="Xóa series"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-background rounded-xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 bg-muted/30 rounded-b-xl border-t border-border">
              ...
              <button
                onClick={
                  modalType === "add" ? handleAddSeries : handleEditSeries
                }
