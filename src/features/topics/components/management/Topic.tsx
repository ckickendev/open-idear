...
const Topic = () => {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        ...
        if (response.success) {
          const topicsList = Array.isArray(response.data?.data)
            ? response.data.data
  ...
  const handleAddTopic = async () => {
    if (!formData.name.trim()) {
      toast.success("Tên chủ đề không được để trống");
      return;
    }
    changeLoad();
    try {
  ...
  const handleEditTopic = () => {
    ...
    changeLoad();
    topicApi
      .updateTopic(selectedItem?._id as string, {
        name: formData.name,
        description: formData.description,
      })
      .then((response) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Quản lý các chủ đề phân loại nội dung
          </p>
        </div>
        <button
      ...
      <p className="text-sm text-muted-foreground">
        ...
        {searchTerm && (
          <span className="text-muted-foreground">
            (lọc từ {topics.length} tổng cộng)
          </span>
        )}
      </p>
      ...
      <div className="bg-background rounded-xl border border-border shadow-admin-sm overflow-hidden">
        {isDataLoading ? (
          ...
          <EmptyState
            icon={Hash}
            title={searchTerm ? "Không tìm thấy chủ đề" : "Chưa có chủ đề nào"}
            description={
              searchTerm
                ? "Thử thay đổi từ khóa tìm kiếm"
                : "Bắt đầu bằng cách thêm chủ đề đầu tiên"
            }
            actionLabel={!searchTerm ? "Thêm chủ đề" : undefined}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            onClick={() => setConfirmDelete(topic._id)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => confirmDelete && handleDeleteTopic(confirmDelete)}
        title="Xóa chủ đề"
