...
const UserList = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (response.success) {
          ...
          toast.success(response.message || "Authentication error!");
        }
      } catch (error: any) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesActivate =
      activateFilter === "all" || user.activate.toString() === activateFilter;
  ...
  const hasActiveFilters =
    searchTerm || roleFilter !== "all" || activateFilter !== "all";
  ...
  const handleEditUser = () => {
    ...
    if (
      ...
      toast.success("Vui lòng điền đầy đủ thông tin bắt buộc");
      changeLoad();
      return;
    }
    userApi
      .updateUser(selectedItem?._id as string, {
      ...
      .then((response) => {
        if (response.success) {
          setUsers(
            users.map((u) =>
              u._id === selectedItem?._id ? response.data.user : u,
            ),
          );
          setShowModal(false);
  ...
  const toggleUserStatus = async (userId: string, currentActivate: string) => {
    ...
    try {
      ...
      setUsers(
        users.map((u) =>
          u._id === userId ? { ...u, activate: newActivate } : u,
        ),
      );
      toast.success(
        `${newActivate === "true" ? "Kích hoạt" : "Vô hiệu hóa"} người dùng thành công`,
      );
    } catch (error: any) {
      toast.error(error?.message);
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-admin-primary rounded-lg hover:bg-admin-primary-hover transition-colors"
        >
      ...
      <div className="bg-background rounded-xl border border-border shadow-admin-sm overflow-hidden">
        {isDataLoading ? (
          ...
          <EmptyState
            ...
            description={hasActiveFilters ? "Thử thay đổi bộ lọc" : undefined}
          />
        ) : (
          <>
                ...
                <tbody className="divide-y divide-border">
                  {currentUsers.map((user, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <StatusBadge
                          variant={user.role === 1 ? "info" : "neutral"}
                        >
                          {user.role === 1 ? (
                      ...
                      <td className="px-6 py-4">
                        <StatusBadge
                          variant={
                            user.activate === true ? "success" : "danger"
                          }
                          dot
                        >
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {listStatus === "trash" ? (
                            ...
                            <>
                              ...
                              <button
                                onClick={() =>
                                  toggleUserStatus(user._id, user.activate)
                                }
                        ...
                        <div className="flex items-center gap-1">
                          {listStatus === "trash" ? (
                            ...
                            <>
                              <button
                                onClick={() => openModal("edit", user)}
                                className="p-1.5 rounded-lg text-admin-primary hover:bg-admin-primary-light"
                              >
                                <Edit size={14} />
                              </button>
                              <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            ...
            onClick={closeModal}
          />
          <div className="relative bg-background rounded-xl shadow-xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={formData.role}
                  className="w-full px-3 py-2.5 text-sm border border-border rounded-lg focus:outline-none focus:border-admin-primary focus:ring-2 focus:ring-admin-primary-ring"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, activate: e.target.value })
                  }
                  className="w-full px-3 py-2.5 text-sm border border-border rounded-lg focus:outline-none focus:border-admin-primary focus:ring-2 focus:ring-admin-primary-ring"
                >
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 bg-muted/30 rounded-b-xl border-t border-border sticky bottom-0">
              <button
                onClick={closeModal}
